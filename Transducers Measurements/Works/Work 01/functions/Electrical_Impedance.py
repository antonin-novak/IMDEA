...
class Electrical_Impedance:
    ...
    Re = 2.9 # [Ohms] # (!!! estimate your own Re from your data !!!)

    # angular frequency
    omega = 2*np.pi*f_axis

    # measured (blocked) electrical impedance Ze
    # Ze = ... you should already have this data from the measurement

    # model of the electrical impedance (an object containing the parameters and methods to reconstruct the impedance curves from the model parameters)
    my_Ze_model = Electrical_Impedance(omega, Ze_measured=Ze, Re_estimated=Re, model='Leach')

    # estimated parameters of Leach model
    print(my_Ze_model.params)

    # impedance from the estimated model
    Ze_model = my_Ze_model.Ze_model(omega)

    fig, ax = plt.subplots()
    ax.semilogx(f_axis, np.real(Ze))          # plot the measured impedance
    ax.semilogx(f_axis, np.real(Ze_model))    # plot the model impedance
